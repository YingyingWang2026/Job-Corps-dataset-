...
SAVE_DIR <- "result_for_SimIII_binary"
...
# estimation base function

compute_Tau_SC <- function(params, data) {
  phi <- as.numeric(params$phi)
  xv <- data$X
  eta1 <- phi[1] + phi[2] * xv + phi[3] + phi[4] * xv
  eta0 <- phi[1] + phi[2] * xv
  EY1 <- mean(plogis(eta1))
  EY0 <- mean(plogis(eta0))
...
compute_Tau_SC_gradient <- function(params, data) {
  phi <- as.numeric(params$phi)
  xv <- data$X
  eta1 <- phi[1] + phi[2] * xv + phi[3] + phi[4] * xv
  eta0 <- phi[1] + phi[2] * xv
...
empty_single_result <- function(error_message = NA_character_) {
  list(
    ...
    Tau_SC_se = NA_real_,
    loglik = NA_real_,
    theta_hat = rep(NA_real_, 12),
    theta_true = rep(NA_real_, 12)
  )
}

run_single <- function(n, data_gen_func, em_func, true_param_func, model_type) {
  tryCatch({
    ...
    theta_hat <- as.numeric(theta_hat)
    theta_true <- as.numeric(c(tp$alpha, tp$phi, tp$gamma, tp$rho))
    grad <- compute_Tau_SC_gradient(result$params, data)
    ...
    valid <- isTRUE(result$converged) && is.finite(ea$Tau_SC) && is.finite(ta$Tau_SC) && is.finite(Tau_SC_var) && Tau_SC_var >= 0 && is.finite(Tau_SC_se)
...
# replication
run_experiment <- function(n_samples, n_sim, data_gen_func, em_func, true_param_func, label, model_type, n_cores, save_dir) {
  all_res <- list()
  
  export_funcs <- c(
    "generate_data_P","compute_Tau_SC", "compute_true_Tau_SC",
    "compute_Tau_SC_gradient","compute_sandwich","empty_single_result",
    "run_single", "initialize_parameters_M_1","E_step_M_1","M_step_M_1",
    "compute_observed_loglik_M_1","compute_individual_loglik_M_1",
    "EM_algorithm_M_1","compute_true_parameters_M_1",
  ...
  for (n in n_samples) {
    cat(sprintf("\nSample size n = %d, number of simulations = %d\n", n, n_sim))
    t0 <- Sys.time()
    ...
    raw <- tryCatch({
      foreach(
        ...
        run_single(n, data_gen_func, em_func, true_param_func, model_type)
      }
    }, finally = {
      close(pb)
    ...
    raw <- lapply(raw, function(r) {
      if (inherits(r, "error")) empty_single_result(conditionMessage(r)) else r
    })
    ...
    cat(sprintf(
      "Coverage %.1f%% | Mean Tau_SC %.4f | Bias %.4f | MC_SD %.4f | Mean_SE %.4f | Mean_Var %.6f\n",
    ...
    all_res[[as.character(n)]] <- list(ace_df = ace_df, param_df = param_df, raw = raw, time = dt)
  }
...
empty_vuong_result <- function(sim_idx, result_M_1, result_M_2, decision, error_message = NA_character_) {
  data.frame(
    sim = sim_idx,
...
run_vuong_type12_from_results <- function(sim_idx, n, result_M_1, result_M_2, data_gen_func, indiv_ll_M_1_func, indiv_ll_M_2_func, total_ll_M_1_func, total_ll_M_2_func) {
  tryCatch({
    if (!isTRUE(result_M_1$success) || !isTRUE(result_M_2$success)) {
      return(empty_vuong_result(
        sim_idx,
        result_M_1,
        result_M_2,
        "ERROR_IN_EM",
        paste(result_M_1$error_message, result_M_2$error_message, sep = " | ")
      ))
    }
    
    if (!isTRUE(result_M_1$converged) || !isTRUE(result_M_2$converged)) {
      return(empty_vuong_result(sim_idx, result_M_1, result_M_2, "NOT_CONVERGED"))
    }
    ...
    B_M_12 <- crossprod(S_M_1, S_M_2) / n_obs
    ...
    empty_vuong_result(sim_idx, result_M_1, result_M_2, "ERROR", conditionMessage(e))
  })
}
...
for (N_SIM in N_SIM_VALUES) {
  ...
  cat("\n*** Step 1: EM_M_1 on data_P ***\n")
  res_M_1 <- run_experiment(N_SIZE, N_SIM, generate_data_P, EM_algorithm_M_1,
                            compute_true_parameters_M_1, "M_1_on_dataP", "M_1", N_CORES, SAVE_DIR)
  cat("\n*** Step 2: EM_M_2 on data_P ***\n")
  res_M_2 <- run_experiment(N_SIZE, N_SIM, generate_data_P, EM_algorithm_M_2,
                            compute_true_parameters_M_2, "M_2_on_dataP", "M_2", N_CORES, SAVE_DIR)
  ...
  export_vars_vuong <- c(
    "ALPHA", "SEED_BASE", "generate_data_P", "compute_scores", "safe_inverse",
    "imhof_pval_from_W", "empty_vuong_result", "loglik_theta_M_1", "loglik_theta_M_2",
    ...
    "theta_to_params_M_1", "theta_to_params_M_2", "run_vuong_type12_from_results"
  )
  ...
  results_raw <- tryCatch({
    foreach(
      ...
      run_vuong_type12_from_results(
        b, N_SIZE, M_1_raw[[b]], M_2_raw[[b]], generate_data_P,
        individual_loglik_theta_M_1, individual_loglik_theta_M_2,
  ...
  results_raw <- lapply(seq_along(results_raw), function(b) {
    ...
    data.frame(
      sim = b, success_M_1 = NA, success_M_2 = NA,
      converged_M_1 = NA, converged_M_2 = NA, pval = NA_real_,
      decision = "PARALLEL_ERROR", LR_n = NA_real_, omega2_n = NA_real_,
      omega_n = NA_real_, Tn = NA_real_, Vn = NA_real_,
      variance_test_reject = NA, n_lambda = NA_integer_, max_abs_imag = NA_real_,
      ifault = NA_integer_, error_message = conditionMessage(r), stringsAsFactors = FALSE
    )
  })
  ...
  simulation_results[[as.character(N_SIM)]] <- list(
    ...
    elapsed_vuong_minutes = as.numeric(difftime(Sys.time(), t_start_vuong, units = "mins"))
  )
}
