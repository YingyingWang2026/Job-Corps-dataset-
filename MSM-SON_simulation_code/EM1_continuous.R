...
##### EM1.R — Model M_1: P(R_2 | R_1, Y_2) for continuous outcome Y_2 

initialize_parameters_M_1 <- function(data, M = 100) {
  # alpha: P(Y_1|X)
  ...
  gamma <- tryCatch({
    ...
    g
  }, error = function(e) c("(Intercept)"=0.8, "X"=0.3, "Y_1_obs"=0.4))
  ...
  rho <- tryCatch({
    ...
    r <- coef(glm(R_2 ~ R_1 + Y_2_imp, data = dr, family = binomial()))
    names(r) <- c("(Intercept)", "R_1", "Y_2_obs")
    r
  }, error = function(e) c("(Intercept)"=0.8, "R_1"=2.5, "Y_2_obs"=0.3))
...
# E step
E_step_M_1 <- function(data, params) {
  ...
  for (i in seq_along(imp)) {
    ...
    if (s$ct == "c1") {
      ...
      for (ss in 1:Mm) {
        ...
        pr2_0 <- 1 - plogis(r_[1]+r_[2]*r1+r_[3]*y2_s)          
      ...
      for (y1 in 0:1) {
        ...
        pr2_1 <- plogis(r_[1]+r_[2]*r1+r_[3]*y2)                
...
M_step_M_1 <- function(wd, imp) {
  ...
  rho <- coef(glm(R_2 ~ R_1 + Y_2, data=df, family=binomial(), weights=w))  
...
compute_observed_loglik_M_1 <- function(data, params) {
  a_ <- params$alpha; p_ <- params$phi
  g_ <- params$gamma; r_ <- params$rho
  ll <- 0; n_quad <- 20
  ...
  for (i in 1:nrow(data)) {
    ...
    if (!is.na(y1o) && !is.na(y2o)) {
      ...
      lp <- log(py1+1e-10) + log(pr1+1e-10) +
        ...
        log(plogis(r_[1]+r_[2]*r1+r_[3]*y2o)+1e-10)       
      ...
      for (y2v in y2r) {
        ...
        pr2_0 <- 1 - plogis(r_[1]+r_[2]*r1+r_[3]*y2v)          
        ps <- ps + fy2 * pr2_0
      }
      ps <- ps * dy2
      ...
      for (y1 in 0:1) {
        ...
        if (!is.na(y2o)) {
          ...
          pr2 <- plogis(r_[1]+r_[2]*r1+r_[3]*y2o)              
...
compute_individual_loglik_M_1 <- function(data, params) {
  a_ <- params$alpha; p_ <- params$phi
  g_ <- params$gamma; r_ <- params$rho
  ...
  for (i in 1:n) {
    ...
    if (!is.na(y1o) && !is.na(y2o)) {
      ...
      y2r <- seq(mu_y2-4*p_$sigma, mu_y2+4*p_$sigma, length.out=n_quad)
      dy2 <- y2r[2]-y2r[1]; ps <- 0
      for (y2v in y2r) ps <- ps + dnorm(y2v,mu_y2,p_$sigma)*(1-plogis(r_[1]+r_[2]*r1+r_[3]*y2v))
      ...
      for (y1 in 0:1) {
        ...
        mu_y2 <- p_$beta[1]+p_$beta[2]*x+p_$beta[3]*y1+p_$beta[4]*x*y1
        if (!is.na(y2o)) {
          ps <- ps + py1*pr1*dnorm(y2o,mu_y2,p_$sigma)*plogis(r_[1]+r_[2]*r1+r_[3]*y2o)
        } else {
          y2r <- seq(mu_y2-4*p_$sigma, mu_y2+4*p_$sigma, length.out=n_quad)
          dy2 <- y2r[2]-y2r[1]; y1_int <- 0
          for (y2v in y2r) y1_int <- y1_int+dnorm(y2v,mu_y2,p_$sigma)*(1-plogis(r_[1]+r_[2]*r1+r_[3]*y2v))
...
# EM algorithm
EM_algorithm_M_1 <- function(data, max_iter=500, tol=1e-4, M=100) {
  params <- initialize_parameters_M_1(data, M=M) 
  lh <- numeric(max_iter)
  for (it in 1:max_iter) {
    wd <- E_step_M_1(data, params) 
    params <- M_step_M_1(wd, params$imp) 
    lh[it] <- compute_observed_loglik_M_1(data, params) 
...
compute_true_parameters_M_1 <- function(data) {
  ...
  list(
    ...
    rho   = coef(glm(R_2 ~ R_1 + Y_2_true, data=data, family=binomial()))
  )
}


#  params conversion 
params_to_theta_M_1 <- function(params) { 
  c(params$alpha, params$phi$beta, params$phi$sigma, params$gamma, params$rho)
}

theta_to_params_M_1 <- function(theta) { 
  list(
    alpha = theta[1:2],
...
loglik_theta_M_1 <- function(theta, data) { 
  compute_observed_loglik_M_1(data, theta_to_params_M_1(theta)) 
}

individual_loglik_theta_M_1 <- function(theta, data) { 
  compute_individual_loglik_M_1(data, theta_to_params_M_1(theta)) 
}
