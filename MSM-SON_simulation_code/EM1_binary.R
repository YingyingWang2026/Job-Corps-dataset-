...
##### EM1.R — Model M_1: P(R_2 | R_1, Y_2) for binary outcome Y_2 

initialize_parameters_M_1 <- function(data) {
 alpha: P(Y_1|X)
 ...
 rho: P(R_2 | R_1, Y_2) 
  rho <- tryCatch(
    coef(glm(R_2 ~ R_1 + Y_2_true, data = data, family = binomial())),
    error = function(e) c("(Intercept)" = 0.5, "R_1" = 0.2, "Y_2" = 0.2)
  )
  ...
  for (i in seq_along(imp)) {
    ...
    if (s$ct == "c1") {
      ...
      for (y in 0:1) {
        ...
        pry <- plogis(r_[1] + r_[2]*r1 + r_[3]*y)           
      ...
      for (a in 0:1) for (y in 0:1) {
        ...
        py  <- plogis(p_[1] + p_[2]*x + p_[3]*a + p_[4]*x*a)
        py  <- ifelse(y == 1, py, 1 - py)
        pry <- plogis(r_[1] + r_[2]*r1 + r_[3]*y); pry0 <- 1 - pry  
...
M_step_M_1 <- function(wd, imp) {
  ...
  rho   <- coef(glm(R_2 ~ R_1 + Y_2,     data = df, family = binomial(), weights = w)) # R_1 and Y_2 are included in model M_1.
...
# EM algorithm 
EM_algorithm_M_1 <- function(data, max_iter = 500, tol = 1e-4) {
  params <- initialize_parameters_M_1(data)
  lh <- numeric(max_iter)
